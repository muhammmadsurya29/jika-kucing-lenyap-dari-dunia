...
func _on_dialogic_text_event(info:Dictionary):
	...
	if info.character in registered_characters:
		...
		var found = false
		if info.character and get_tree() and get_tree().current_scene:
			var char_name = info.character.resource_path.get_file().get_basename()
			if char_name.to_lower() == "mc": char_name = "player"
			var found_node = get_tree().current_scene.find_child("*" + char_name + "*", true, false)
			if found_node:
				node_to_point_at = found_node
				$Example.hide()
				found = true
		if not found:
